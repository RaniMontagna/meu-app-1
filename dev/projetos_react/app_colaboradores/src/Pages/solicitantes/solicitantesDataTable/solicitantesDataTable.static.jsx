...
export const HeaderWithSearch = ({ setAdicionarEditarSolicitante }) => {
  const { setFiltrosDataTable, filtrosDataTable } = useSolicitantesContext();
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      <div style={{ display: 'flex', gap: '8px' }}>
        ...
        <span className="p-input-icon-left">
          ...
          <InputText value={search} onChange={_handleOnChangeSearch} placeholder="Buscar por solicitante" />
        </span>
      </div>
      <div>
        <Button
          ...
          className="p-button-outlined"
          onClick={() => setAdicionarEditarSolicitante({ open: true })}
        />
      </div>
    </div>
  );
